build {
  ...
  provisioner "shell" {
    ...
    remote_folder = "/opt/packer/files"
    inline = [
      "echo '8.8' | sudo tee /etc/yum/vars/releasever"
    ]
  }
  provisioner "shell" {
    only = [
    ...
    inline = [
      ...
      "sudo dnf install -y unzip wget jq sshpass",
      "sudo dnf install -y python3.11 python3.11-pip python3.11-setuptools python3.11-libs python3.11-devel",
      ...
      "sudo pip3.11 install --upgrade boto3 requests",
      "sudo dnf upgrade -y"
    ]
  }
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "sudo yum install -y gcc bzip2-devel libffi-devel sshpass openssl11-devel",
      ...
      "cd /opt",
      "sudo rm -rf /opt/Python-3.11.5.tgz /opt/Python-3.11.5",
      "python3.11 -V",
      ...
      "sudo /usr/local/bin/pip3.11 install --upgrade cryptography"
    ]
  }
  provisioner "shell" {
    only = [
    ...
    inline = [
      ...
      "pip3.11 install --upgrade requests pyopenssl boto3 botocore",
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "sudo pip3.11 install --upgrade boto3 botocore jmespath",
      "sudo mkdir -p /usr/share/ansible/collections",
      "sudo chown -R ${local.aws_ssh_username}:root /usr/share/ansible/collections",
      "ansible-galaxy collection install amazon.aws -p /usr/share/ansible/collections",
      "ansible-galaxy collection install community.aws -p /usr/share/ansible/collections",
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "sudo pip3.11 install --upgrade cryptography boto3 botocore"
    ]
  }
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "python3.11 -m venv ansible",
      "source /opt/venv/ansible/bin/activate",
      "pip3.11 install ansible-core==2.13.3",
      "pip3.11 install --upgrade requests boto3 botocore pyopenssl",
      "ansible-galaxy collection install amazon.aws -p /usr/share/ansible/collections",
      "ansible-galaxy collection install community.aws -p /usr/share/ansible/collections",
      "ansible-galaxy collection install community.general -p /usr/share/ansible/collections",
      "ansible-galaxy collection install community.crypto -p /usr/share/ansible/collections",
  ...
  provisioner "file" {
    ...
    destination = "/opt/packer/files/dod_CAs.pem"
    source      = "./files/linux/dod_CAs.pem"
  }
  provisioner "shell" {
    only = [
      "amazon-ebs.rhel-stig"
    ...
    remote_folder = "/opt/packer/files"
    inline = [
      "sudo cp /opt/packer/files/dod_CAs.pem /etc/pki/ca-trust/source/anchors/dod_CAs.pem",
      "sudo cp /opt/packer/files/dod_CAs.pem /etc/sssd/pki/sssd_auth_ca_db.pem",
  ...
  provisioner "shell" {
    ...
    remote_folder = "/opt/packer/files"
    inline = [
      "sudo cp /opt/packer/files/dod_CAs.pem /usr/local/share/ca-certificates/dod_CAs.pem",
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "sudo pip3.11 uninstall -y ansible-core"
    ]
  }
